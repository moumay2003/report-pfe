...
The solution comprises two engineered artefacts that together address objectives O1 and O2 defined in Chapter~1 (environment audit and synchronisation strategy). Their roles are summarised in Table~\ref{tab:design-objectives}.
...
\textbf{O1}: Audit and quantification of schema and data differences across environments \\
Synchronisation Strategy &
Controlled remediation using Secure Data Sharing &
\textbf{O2}: Synchronisation of \texttt{DEV} and \texttt{UAT} with \texttt{PROD} \\
\bottomrule
\end{tabularx}
...
The following subsections present the architecture through three design views---an architecture overview, the persistence data model, and the dynamics of a comparison run---followed by a description of the object types compared and the criteria applied to each.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{comparator_architecture.png}
  \caption{Architecture overview of the Snowflake Environment Comparator, from read-only connection to the source and target accounts, through metadata extraction and local storage, to the comparison engine and the reporting dashboard.}
  \label{fig:sys-context}
\end{figure}

As shown in Figure~\ref{fig:sys-context}, the comparator is organised as an end-to-end pipeline of five stages: it connects read-only to the \emph{source} and \emph{target} Snowflake accounts; extracts object metadata through the Snowflake Python connector; persists every run, result, and drift record in a local SQLite store via SQLAlchemy; compares the two environments in its comparison engine; and surfaces the outcome through a reporting dashboard. The application runs on a single local host operated by one data engineer, so it requires no server infrastructure, no elevated service account, and no standing footprint inside the ARYZTA environments. The most important property, highlighted in the security-and-governance band of the diagram, is that every interaction with Snowflake is \emph{read-only}: the application issues only \texttt{SELECT}, \texttt{SHOW}, and \texttt{DESCRIBE} statements and never emits \gls{ddl}, \gls{dml}, \texttt{CALL}, \texttt{USE}, or \texttt{ALTER}. This guarantee is enforced structurally---by a client-side allow-list that rejects any non-read statement before it can be transmitted---which is what makes the tool safe to point directly at the production account.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{sync_strategy_architecture.png}
  \caption{Multi-environment synchronisation strategy. \texttt{PROD} is the authoritative provider; its in-scope databases are exposed as read-only Secure Data Shares, and a daily orchestration task drives the synchronisation procedure that realigns the \texttt{UAT} and \texttt{DEV} consumer accounts.}
