...
\begin{figure}[H]
  ...
  \caption{The inbound Secure Data Shares as seen from a consumer account. Each in-scope \texttt{PROD} database is exposed as a dedicated read-only share that the lower environments mount locally for synchronisation.}
  \label{fig:data-share}
\end{figure}

Figure~\ref{fig:data-share} shows the Secure Data Shares provisioned for the project, viewed from a consumer account's external-sharing catalogue. Each in-scope \texttt{PROD} database is published as its own read-only share---visible here under names following the project's share convention---and mounted as a local database in the \texttt{UAT} and \texttt{DEV} accounts. One of the shares is annotated ``Alignment PROD--UAT'', reflecting its purpose. This realises the provider/consumer architecture of Chapter~3: production data is exposed live and read-only, without being copied, and remains the untouched source of truth.
...
\begin{figure}[H]
  ...
  \caption{A run of the synchronisation procedure in Snowflake. The procedure is invoked for a share-mounted source schema and a local target schema, and returns a log table recording, per table, the action taken and its outcome.}
  \label{fig:sync-run}
\end{figure}
